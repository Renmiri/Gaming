...
namespace ME2DLCUnlocker
{
    class Program
    {
        ...
        private static void WriteLine(string ln, string rplce1)
        {
            WriteLine(ln.Replace("{0}", rplce1));
        }

        private static void WriteLine(string ln, string rplce1, string rplce2)
        {
            WriteLine(ln.Replace("{1}", rplce1).Replace("{0}", rplce2));
        }

        private static byte[] IPEncryptor()
        {
        ...
        {
            WriteLine("ME2DLCUnlocker by Warranty Voider");
            WriteLine("Based upon the cracks by Illiria and Loghain");
            WriteLine();
            string loc = Environment.CurrentDirectory + "\\";
            string pathdlc = loc + @"..\BioGame\DLC\";
            WriteLine("Location : " + loc);
            if (!Directory.Exists(pathdlc) || !File.Exists(loc + "MassEffect2.exe"))
            {
                WriteLine("Error, please put in folder where MassEffect2.exe is");
                return;
            }
            WriteLine("Looking for DLC...");
            StringWriter writer = new StringWriter();
            writer.WriteLine("[Hash]");
            foreach (string dlc in Directory.GetDirectories(pathdlc))
            {
            ...
            if (magic == null)
            {
                WriteLine("Could not get adapter entropy.");
                return;
            }
            else
            {
                ...
                if (bytes == null)
                {
                    WriteLine("Could not encrypt entitlement cache.");
                    return;
                }
                else
                {
            ...
            proc.StartInfo.FileName = "MassEffect2.exe";
